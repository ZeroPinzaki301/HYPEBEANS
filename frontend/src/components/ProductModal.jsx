...
const ProductModal = ({ closeModal, saveProduct, editingProduct }) => {
  const [product, setProduct] = useState({
    ...
    beverageType: "",
    imageFile: null,
    ingredients: [], // Ensure array is properly formatted
  });
  ...
  const handleSubmit = (e) => {
    ...
    // Ensure ingredients are sent correctly as a JSON string
    const formattedProduct = {
      ...product,
      ingredients: JSON.stringify(product.ingredients),
    };

    saveProduct(formattedProduct, product.imageFile);
    closeModal();
  };
          ...
          <div className="mb-4">
            ...
            {product.ingredients.map((ing, i) => (
              <div key={i} className="flex items-center mb-2 gap-2">
                <input type="text" placeholder="Ingredient Name" value={ing.name} onChange={(e) => handleIngredientChange(i, "name", e.target.value)} className="w-2/3 p-2 border rounded-lg" required />
                <input type="number" min="1" placeholder="Qty" value={ing.quantity} onChange={(e) => handleIngredientChange(i, "quantity", e.target.value)} className="w-20 border p-1 rounded-lg" required />
                <select value={ing.unit} onChange={(e) => handleIngredientChange(i, "unit", e.target.value)} className="w-20 border p-1 rounded-lg">
                  <option value="pcs">pcs</option>
                  <option value="g">g</option>
                  <option value="ml">ml</option>
                </select>
                <button type="button" onClick={() => removeIngredientField(i)} className="bg-red-500 text-white px-3 py-1 rounded-lg">X</button>
              </div>
            ))}
